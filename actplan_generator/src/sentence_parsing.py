...
class SentenceParsing():
    ...
    #正しい文の作成
    '''
    def wordVerification(self,current_quetion:int,sentence:str,info_exd=False)-> str:
        set_quetion=self.question_ls[current_quetion]  #｛｝に文字列を入れたquetion用
        tag_flag=False  #{}のみ編集するため
        str_position=0  #代入したリストの要素の位置調整
        str_mk1=""
        str_mk2=""
        tag_ls=[]
        word_ls=[]
        d=dif.SequenceMatcher(None,self.question_ls[current_quetion],sentence)
        #中括弧の部分だけ文章に取り組む
        for tag,i1,i2,j1,j2 in d.get_opcodes():
            if("{" in self.question_ls[current_quetion][i1:i2]):
                tag_flag=True
            if(tag_flag):
                if("}" in self.question_ls[current_quetion][i1:i2]):
                    tag_ls.append(str_mk1)
                    word_ls.append(str_mk2)
                    str_mk1=""
                    str_mk2=""
                    tag_flag=False
                i1+=str_position
                ...
                if (tag=="equal"):
                    ...
                    str_mk1+=set_quetion[i1:i2]
                    str_mk2+=sentence[j1:j2]
                    set_quetion=set_quetion[:i1]+sentence[j1:j2]+set_quetion[i2:]

                str_position+=(j2-j1)-(i2-i1)
        tag_ls.append(str_mk1.replace("\n",""))
        word_ls.append(str_mk2.replace("\n",""))
        #誤認識対策
        for num,(tag,word) in enumerate(zip(tag_ls,word_ls)):
            ed_flag=True
            if tag=="{object}":
                current_ls=[j for i in self.tag_data["category"].values() for j in i["object"]]
                current=self.levSearch(word,current_ls)
            elif tag=="{category}":
                current_ls=[i for i in self.tag_data["category"]]
                current=self.levSearch(word,current_ls)
            elif tag=="{location}":
                current_ls=[j for i in self.tag_data["room"].values() for j in i]
                current=self.levSearch(word,current_ls)
            elif tag=="{room}":
                current_ls=[i for i in self.tag_data["room"]]
                current=self.levSearch(word,current_ls)
            elif tag=="{gestures}":
                current_ls=[i for i in self.tag_data["gestures"]]
                current=self.levSearch(word,current_ls)
            elif tag=="{name}":
                current_ls=[j for i in self.tag_data["gender"].values() for j in i]
                current=self.levSearch(word,current_ls)
            else:
                ed_flag=False
            ...
            if ed_flag:
                if self.category3:
                    ...
                    if current==-1:
                        return ""
                    set_quetion=set_quetion.replace(word,current_ls[current])
                    word_ls[num]=current_ls[current]
        #print(self.question_ls[current_quetion])
        #print(set_quetion)
        tag_ls.pop(0)
        word_ls.pop(0)
        self.tag_ls,self.word_ls=(tag_ls,word_ls)
        #print(self.tag_ls,self.word_ls)
        return set_quetion.replace("\n","")
    '''
