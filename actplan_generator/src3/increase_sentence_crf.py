...
class Increase_Sentence():
    def __init__(self):
        ...
        self.act_give = ["give","put","serve","provide"]
        self.act_grasp = ["get","grasp","bring","pick","deliver","take"]
    ...
    def compare_similarity_action(self,word):
        word_list = ["go","navigate","tell","give","grasp"]
        word_dict = {}
        result = []
        max_value = []
    ...
    def random_generate_fix(self, root):
            buffer = ""
            pos = 0
            num_act = 1
            num_loc = 1
            ...
            for sentence in root:
                ...
                if sentence.tag == "act":
                    ...
                    #result = self.compare_similarity_target(sentence.text) 
                    if result == 'object':
                        #target = random.choice(self.tar_object)
                        pass
                    elif result == 'day':
                    ...
                    if sentence.text not in self.human_other:
                        human = random.choice(self.human)
                    else:
                        human = sentence.text
    ...
    def sample_execute(self):
        ...
        root1 = xml.etree.ElementTree.fromstring("<dummy>"+line1+"</dummy>")
        root2 = xml.etree.ElementTree.fromstring("<dummy>"+line2+"</dummy>")
        root3 = xml.etree.ElementTree.fromstring("<dummy>"+line3+"</dummy>")
        
        for i in range(1):
            sen1, posdict = self.random_generate_fix(root1)
            sen2, posdict = self.random_generate_fix(root2)
            sen3, posdict = self.random_generate_fix(root3)
            
            print("sen1_before:",example_sentence1)
            print("sen2_before:",example_sentence2)
            print("sen3_before:",example_sentence3)
            print("-----------------------------------")    
            print("sen1:",sen1)
            print("sen2:",sen2)
            print("sen3:",sen3)
